 ...
 * @property {string} provider.type - An identifier for the type of network selected, allows DekuSan to use custom provider strategies for known networks.
 ...
 * @property {DekuSanState} data - The data emitted from DekuSan controller, or used to initialize it.
 * @property {Number} version - The latest migration version that has been run.
 */

/**
 * Initializes the DekuSan controller, and sets up all platform configuration.
 ...
 * @returns {Promise<DekuSanState>} Last data emitted from previous instance of DekuSan.
 */
...
>>>>>>> Complete onboarding flow
  ...
  if (!versionedData) {
    throw new Error('DekuSan - migrator returned undefined')
  }
  ...
  if (localStore.isSupported) {
    ...
    setTimeout(() => {
      throw new Error('DekuSan - Localstore not supported')
    })
  }
...
/**
 * Initializes the DekuSan Controller with any initial state and default language.
...
function setupController (initState, initLangCode) {
  ...
  pump(
    ...
    (error) => {
      log.error('DekuSan - Persistence pipeline failed', error)
    }
  )

  /**
   * Assigns the given state to the versioned object (with metadata), and returns that.
   * @param {Object} state - The state object as emitted by the DekuSanController.
  ...
  async function persistData (state) {
    if (!state) {
      throw new Error('DekuSan - updated state is missing', state)
    }
    if (!state.data) {
      throw new Error('DekuSan - updated state does not have data', state)
    }
    if (localStore.isSupported) {
      try {
  ...
  /**
   * Connects a Port to the DekuSan controller via a multiplexed duplex stream.
   * This method identifies trusted (DekuSan) interfaces, and connects them differently from untrusted (web pages).
...
>>>>>>> Complete onboarding flow
      ...
      controller.setupTrustedCommunication(portStream, 'DekuSan')
