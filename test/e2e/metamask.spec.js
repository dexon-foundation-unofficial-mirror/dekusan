...
describe('Metamask popup page', function () {
  ...
  describe('Account Creation', () => {
    ...
    it('matches DekuSan title', async () => {
      const title = await driver.getTitle()
      assert.equal(title, 'DekuSan', 'title matches DekuSan')
      await delay(300)
    })
  ...
  describe('Token Factory', function () {
    ...
    it('confirms transaction in DekuSan popup', async function () {
      const windowHandles = await driver.getAllWindowHandles()
    ...
    it('navigates back to DekuSan popup in the tab', async function () {
      await driver.get(extensionUri)
      await delay(700)
    })
  })
